<?php
    if(isset($_SESSION['uname'])){
        ...
        if($result[0] > 0){
            ...
            while($response = $query->fetch()){
                $i++;
                echo '<li><h2><a id="item' . $response['id'] . '" onclick="load_item(\'' . $response['id'] . '\');">' . $response['title'] . '</a></h2>' . $name[$response['feed_id']] . '</li>';
            }
        }
        else{
            echo '<li><h2><a>Vous n\'avez rien à lire</a></h2>Lancez donc une mise à jour des flux!</li>';
            $i = 0;
        }
